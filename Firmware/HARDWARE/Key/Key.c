...
void Key_Loop(void)
{
	...
	if(LastState==1 && NowState==1 && Key_A0.Time_1 <= 1500 && Key_A0.LongPressOpen_Flag)	//按键持续下压，长按时间累加（1.5s）
	{
		Key_A0.Time_1 += 20;	//加20是因为TIM2定时20ms
	}
	
	if(LastState == 1 && Key_A0.Time_1 > 1500 && Key_A0.LongPressOpen_Flag)				//长按1.5s
	{
